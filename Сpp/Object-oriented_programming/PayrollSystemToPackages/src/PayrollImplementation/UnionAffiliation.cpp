...
void UnionAffiliation::AddItem(Date& date, double value, Affiliation* thisObject)
{
	if (UnionAffiliation* uaf = dynamic_cast<UnionAffiliation*>(thisObject))
	{
		uaf->AddServiceCharge(new ServiceCharge(date, value));
	}
	else
	{
