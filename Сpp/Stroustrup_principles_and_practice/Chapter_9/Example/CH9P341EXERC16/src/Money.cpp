...
namespace MyLib
{
	...
	std::ostream& operator<<(std::ostream& os, const Money& money)
	{
		...
		return os << " " << (money.amount() / 100) << "." << std::setw(2) << std::right << std::setfill('0') <<(money.amount() % 100);
	}
	...
	std::istream& operator>>(std::istream& is, Money& money)
	{
		...
		if (currency == "USD")
		{
			typeCurrency = Currency::USD;
		}
		else if (currency == "EUR")
		{
			typeCurrency = Currency::EUR;
		}
		else if (currency == "CNY")
		{
			typeCurrency = Currency::CNY;
		}
		else if (currency == "RUB")
		{
			typeCurrency = Currency::RUB;
		}
		else
		{
			throw Money::Invalid();
		...
		money = Money(rounding_to_hundredths(amount), typeCurrency);
	}
}