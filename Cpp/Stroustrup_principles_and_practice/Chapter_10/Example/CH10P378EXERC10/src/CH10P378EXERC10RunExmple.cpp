...
/*-----------------------------------------------------------------------------
comments:
	-	Current moment "from {name file}" and "to {name file}" will not correctly
		work if we try to use them in a file that will be called from cin using "from {name file}"
-----------------------------------------------------------------------------*/
...
/*
	...
	Input from stream_manager; output to stream_manager.
...
#include "CH10P378EXERC10RunExmple.h"
#include "std_lib_facilities.h"

RunCommandOfExample* RunCommandOfExample::runCommandOfExample = new CH10P378EXERC10RunExmple();
...
namespace {
	...
	/*
	* Responsible for the streams for the token stream and the enter stream.
	* @see Token_stream
	*/
	...
	public:
		...
		*/
		...
		/*
		*/
		bool is_processing_ofstream();

		/*
		*/
		bool is_processing_ifstream();

		/*
		*/
		void set_istream(const string& name_istream);

		/*
		*/
		void set_ostream(const string& name_ostream);
	...
	private:
		/* contain handle of input file */
		ifstream mifst;

		/* contain handle of output file*/
		ofstream mofst;
	};
	...
	* This class has the ability to receive from stream (Stream_manager)
	* and return tokens to the stream (Stream_manager).
	*
	...
	*/
	...
	public:
		...
		*/
		Token_stream(Stream_manager* sm);
	...
	private:
		...
		/**/
		Stream_manager* mStream_manager;
	};
	...
	*/
	...
	/*
	* will try to handle file requests and if whole correctly
	* it will change the stream manager
	*/
	...
	*/
	...
	Token_stream token_stream(&stream_manager);
	...
	bool Stream_manager::is_processing_ofstream()
	{
		return mofst.is_open() && mofst;
	}

	bool Stream_manager::is_processing_ifstream()
	{
		return mifst.is_open() && mifst;
	}

	void Stream_manager::set_istream(const string& name_istream)
	{
		if (mifst.is_open())
		{
			mifst.close();
		...
		if (name_istream != "cin")
		{
			mifst.open(name_istream, ifstream::in);
			if(!mifst.is_open())
			{
				error("can't open output file ", name_istream);
			}
		}
	}

	void Stream_manager::set_ostream(const string& name_istream)
	{
		if (mofst.is_open())
		{
			mofst.close();
		...
		if (name_istream != "cout" && name_istream != "cerr")
		{
			mofst.open(name_istream, ifstream::out);
			if (!mofst.is_open())
			{
				error("can't open output file ", name_istream);
			}
	...
	istream& Stream_manager::get_ist()
	{
		if (mifst.is_open())
		{
			if (mifst)
			...
			mifst.close();
			get_ost() << "The current input file is closed\n";
		}
		return cin;
	}

	ostream& Stream_manager::get_ost()
	{
		if (mofst.is_open())
		{
			if (mofst)
			...
			mofst.close();
			get_ost() << "The current output file is closed\n";
		}
		return cout;
	}

	ostream& Stream_manager::get_eost()
	{
		if (mofst.is_open())
		{
			if (mofst)
	...
	Token_stream::Token_stream(Stream_manager* sm)
		: full(false)
		, buffer(Token('\0'))
		, mStream_manager(sm)
	{
	...
	Token Token_stream::get()
	{
		...
		mStream_manager->get_ist() >> noskipws >> ch;
		if (ch < 0)
		{
		...
		while (true) {
			...
			case '9':
			{
				...
				mStream_manager->get_ist().putback(ch);
				double val;
				mStream_manager->get_ist() >> val;
			...
			default:
			{
				...
				if (isalpha(ch))
				{
					string s;
					s += ch;
					while (mStream_manager->get_ist().get(ch) && (isalpha(ch) || isdigit(ch) || ch == '_'))
					{
						s += ch;
					}
					mStream_manager->get_ist().putback(ch);
	...
	void Token_stream::ignore(char c)
	{
		...
		char ch = 0;
		while (mStream_manager->get_ist() >> noskipws >> ch)
		{
			if (ch == c || ch == '\n')
			{
				return;
	...
	double primary()
	{
		...
		while (true) {
			...
			case NAME_KIND:
			{
				...
				return check_token_to_factorial(symbol_table.get(name));
			}
			default:
	...
	void clean_up_mess()
	{
		token_stream.ignore(PRINT_KIND);
	}

	void dispay_help_inforamtion()
	{
		stream_manager.get_ost() << "The following operations are available:\n"
	...
	{
		Token token = token_stream.get();
		if (token.name == FROM_FILE)
		{
			token = token_stream.get();
			if (token.kind == NAME_KIND)
			{
				if (stream_manager.is_processing_ofstream() || stream_manager.is_processing_ifstream())
				{
					stream_manager.get_ost() << "from " << token.name << "\n";
				}
				stream_manager.set_istream(token.name);
				return;
			}
		}
		if (token.name == TO_FILE)
		{
			token = token_stream.get();
			if (token.kind == NAME_KIND)
			{
				stream_manager.set_ostream(token.name);
				return;
			}
		}
		error("Non correct command for handle_file_request\n");
	}
	...
	void calculate()
	{
		...
		stream_manager.get_ost() << "Welcome to the calculator!\nEnter floating point expressions.\n";
		dispay_help_inforamtion();

		while (stream_manager.get_ist())
		{
			stream_manager.get_ost() << PROMPT;
			Token token = token_stream.get();
			...
			try
			{
				...
				stream_manager.get_ost() << RESULT << statement() << '\n';
				statementDone = true;
			}
			catch (exception& e)
			{
				stream_manager.get_eost() << e.what() << '\n';
				clean_up_mess();
				statementDone = false;
			}
		}
...
int CH10P378EXERC10RunExmple::excute()
{
	try
	{
		// predefine names:
