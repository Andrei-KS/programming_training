...
#include "CH20P754EXERC6RunExmple.h"
#include "std_lib_facilities.h"

RunCommandOfExample* RunCommandOfExample::runCommandOfExample = new CH20P754EXERC6RunExmple();
...
int CH20P754EXERC6RunExmple::excute()
{
  Document doc;
  
  stringstream ss{ "1111\n222\n333ab33\n44444\n" };
  ...
  Text_iterator sit = doc.search(pattern);
  if (sit != doc.end())
  {
  ...
  {
    cout << pattern << " isn't found\n";
  }

  cout << "\n next step\n";
  doc.erase(sit, pattern.size());
  print(doc);

  cout << "\n next step\n";
  Text_iterator iit = doc.begin();
  ...
  pattern = string("fgo");
  doc.insert(iit, pattern);
  print(doc);

  cout << "\n next step\n";
  doc.replace("fgo", "table");
  print(doc);
